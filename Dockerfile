...
RUN mkdir -p ~/pg/data; mkdir -p ~/pg/scripts; mkdir -p ~/pg/logs; mkdir -p ~/p$HOMEg/sockets; initdb -D pg/data/
RUN echo '#!/bin/bash\n\
...
# We need nightly build on 2018-04-14 (meant for the day after that)
...
# Give back control for further configuration
USER root
