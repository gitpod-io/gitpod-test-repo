...
import { ReposCompareCommitsParams } from "@octokit/rest";
...
import { Repository, PullRequestReviewComment, PullRequestReview, PullRequest, PullRequestReviewEvent, PullRequestTimelineItem, DraftPullRequestReviewComment } from "./github-protocol";
...
export class GitHubModel {
    ...
    @inject(ForksLoader) protected readonly forksLoader: ForksLoader;
    ...
    async createPullRequest(params: CreatePullRequestParams): Promise<void> {
        ...
        const result = await this.restApi.run<{ number: number }>(api => api.pullRequests.create({
            ...compareParams,
            title: params.title,
            body: params.body,
    ...
    async getMyLogin(): Promise<string> {
        const me = await this.restApi.run<{ login: string }>(async api => api.users.get({}));
        return me.login;
    }

    async getAuthorizedOrganizations(): Promise<string[]> {
        const result = await this.restApi.run<{ login: string }[]>(async api => api.users.getOrgs({}));
        return result.map(o => o.login);
    }

    async hasWritePermission(owner: string, repo: string): Promise<boolean> {
        const response = await this.restApi.run<{ permissions: { push: boolean } }>(async api => api.repos.get({ owner, repo }));
        return response.permissions.push;
    }
    ...
    async submitPullRequestReview(params: SubmitPullRequestReviewParams): Promise<void> {
        if (this.pendingPullRequestReview) {
            this.submitPendingPullRequestReview(params);
        } else {
            this.addPullRequestReview(params);
        }
    }
    ...
    protected async doAddPullRequestReview(params: AddPullRequestReviewParams = {}): Promise<void> {
        ...
        const { data } = await this.endpoint.addPullRequestReview({ pullRequestId, ...params });
        const pullRequestReview = data.addPullRequestReview.pullRequestReview;
    ...
    protected async doSubmitPendingPullRequestReview(params: SubmitPendingPullRequestReviewParams): Promise<void> {
        ...
        const { data } = await this.endpoint.submitPullRequestReview({ pullRequestReviewId, ...params });
        const pullRequestReview = data.submitPullRequestReview.pullRequestReview;
        this._pendingPullRequestReview = PullRequestReview.getPending(pullRequestReview);
        this.fireDidChange();
    }
    ...
    protected async doDeletePendingPullRequestReview(): Promise<void> {
        if (!this.pendingPullRequestReview) return;
        const pullRequestReviewId = this.pendingPullRequestReview.id;
        const { data } = await this.endpoint.deletePullRequestReview({ pullRequestReviewId });
        const pullRequestReview = data.deletePullRequestReview.pullRequestReview;
        this._pendingPullRequestReview = PullRequestReview.getPending(pullRequestReview);
        this.fireDidChange();
    }
    ...
    protected async doAddPendingPullRequestReviewComment(params: AddPendingPullRequestReviewCommentParams): Promise<void> {
        ...
        const { data } = await this.endpoint.addPullRequestReviewComment({ body, inReplyTo, pullRequestReviewId, path, position });

        this.data.pushComment(data.addPullRequestReviewComment.comment);
        this.fireDidChange();
    }
    ...
    protected async doUpdatePullRequestReviewComment(params: UpdatePullRequestReviewCommentParams): Promise<void> {
        ...
        const { data } = await this.endpoint.updatePullRequestReviewComment({ body, pullRequestReviewCommentId })

        this.data.pushComment(data.updatePullRequestReviewComment.pullRequestReviewComment);
        this.fireDidChange();
    }
    ...
    protected async doDeletePullRequestReviewComment(params: DeletePullRequestReviewCommentParams): Promise<void> {
        ...
        await this.restApi.run(api => api.pullRequests.deleteComment({ owner, repo, id }));
    ...
    protected async findPullRequestNumber(): Promise<number | undefined> {
        ...
        const result = await this.restApi.run<{ number: number }[]>(api => api.pullRequests.getAll({ ...params, per_page: 1 }));
        return result[0] && result[0].number;
    }
    ...
    protected refreshTimeline(ghData: GitHubData, loader: BatchLoader): void {
        ...
        const batch = (options: GitHubEndpoint.PullRequestOptions) => {
            loader.batch<PullRequest>(this.endpoint.createGetTimelineQuery(options), ({ timeline }) => {
    ...
    protected refreshReviews(ghData: GitHubData, loader: BatchLoader): void {
        ...
        const batch = (options: GitHubEndpoint.PullRequestOptions) => {
            loader.batch<PullRequest>(this.endpoint.createGetReviewsQuery(options), ({ reviews }) => {
    ...
    protected refreshReviewComments(review: PullRequestReview, ghData: GitHubData, loader: BatchLoader): void {
        const batch = (options: GitHubEndpoint.ReviewOptions) => {
            loader.batch<PullRequestReview>(this.endpoint.createGetReviewCommentsQuery(options), ({ comments }) => {
    ...
    renderMarkdown(text: string): Promise<string> {
        ...
        return this.restApi.run(api => api.misc.renderMarkdown({
            text,
            mode: "gfm",
            context
        }));
     ...
     */
    protected restCompareParams({ includeRenamed }: { includeRenamed: boolean } = { includeRenamed: false }): ReposCompareCommitsParams | undefined {
        if (this.compareParams) {
            const { owner, name } = this.compareParams.base.repository;
    ...
    protected async compare(): Promise<ReposCompareCommitsResult> {
        ...
        try {
            ...
            await this.git.fetch(repository, { remote: baseRemote });
            if (base.crossRepository) {
                await this.git.fetch(repository, { remote: headRemote });
            }
            const baseRevision = `${baseRemote}/${base.name}`;
            const headRevision = `${headRemote}/${head.name}`;
    ...
    protected async getBase({ base, head }: GitHubModel.CompareRefreshParams): Promise<GitHubModel.RawRef | undefined> {
        const headParent = await this.getParentRepository(head);
        if (!headParent) {
            return undefined;
        }
        if (base) {
            const headSource = headParent.isFork ? await this.getSourceRepository({
                owner: headParent.owner.login,
                repository: headParent.name
            }) : headParent;
            const baseSource = await this.getSourceRepository(base);
            if (headSource && baseSource && Repository.equals(headSource, baseSource) && this.existBranch(base)) {
                return base;
            }
        }
        return {
            owner: headParent.owner.login,
            repository: headParent.name,
            refName: Repository.getDefaultBranch(headParent)
        };
    }
    ...
    protected forks: Promise<GitHubModel.Repository[]> = Promise.resolve([]);
    getForks(): Promise<GitHubModel.Repository[]> {
        return this.forks;
    }
    protected async resolveForks(): Promise<GitHubModel.Repository[]> {
        const base = this.compareParams && this.compareParams.base;
        ...
        return this.forksLoader.getForks(owner, name, true);
    }

    async getBranches(repository: GitHubModel.Repository): Promise<string[]> {
        const result: string[] = [];
        await this.endpoint.fetchForward(async cursor => {
            const { data } = await this.endpoint.getBranches({
                cursor,
    ...
    protected async getSourceRepository(options: GitHubEndpoint.RepositoryOptions): Promise<Repository | undefined> {
        const repository = await this.getParentRepository(options);
        if (repository && repository.isFork) {
            return this.getSourceRepository({
                owner: repository.owner.login,
                repository: repository.name
            });
        }
    ...
    protected async getParentRepository(options: GitHubEndpoint.RepositoryOptions): Promise<Repository | undefined> {
        const { data } = await this.endpoint.getParentRepository(options);
        const repository = data.repository;
        return repository && repository.parent || repository || undefined;
    }
    ...
    protected async findMergeBase(): Promise<string | undefined> {
        try {
            ...
            const remote = await this.findRemote(repository, base.repository.fullName);
            if (!remote) {
                return undefined;
            }
            await this.git.fetch(repository, { remote });
            const result = await this.git.exec(repository, ['merge-base', `${remote}/${base.name}`, 'HEAD']);
    ...
    async merge({ mergeMethod }: {
        ...
        await this.restApi.run(api => api.pullRequests.merge({
            number, owner, repo,
...
export namespace GitHubModel {
    ...
    export class Ref {
        ...
        static fromRaw(ref: RawRef, crossRepository: boolean): Ref {
            return new Ref(ref.refName, new GitHubModel.Repository(ref.repository, ref.owner), crossRepository);
        }
        static fromProtocol(ref: protocol.Ref, crossRepository: boolean): Ref {
            return new GitHubModel.Ref(ref.name, new GitHubModel.Repository(ref.repository.name, ref.repository.owner.login), crossRepository);
        }
    }
